...
<div class="row">
	<div class="col-md-12">
		<div class="box box-primary">
			...
			<div class="box-body">
				<form action="{{route('case_mast.store')}}" method="post" autocomplete="off">
				@csrf
					...
					<div class="row from-group" style="" >
						<div class="col-md-6" style="margin-top: 10px; display: none; " id="case_type_div">
							...
							<select class="form-control" name="case_type_id">
								...
								<option value="{{$case_type->case_type_id}}" {{old('case_type_id')==$case_type->case_type_id ? 'selected' : ''}}> {{$case_type->shrt_desc}} ({{$case_type->case_type_desc}})</option>									    
								@endforeach
							</select>
							@error('case_type_id')
								<span class="invalid-feedback text-danger" role="alert">
...
{{-- var old_users_id = "{{collect(old('users_id'))}}"; --}}
// alert(old_users_id)
