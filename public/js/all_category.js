...
function team_users(team_id,auth_id,auth_name,old_users_id = []){
...
function service_mast_fetch(service_mast_id,service_catg_code,old_service_code=''){
	// console.log(service_mast_id)
	$.ajax({
		type:'GET',
		url:'/service_mast_fetch/'+service_catg_code,
		success:function(res){
			
			if(res.length !=0){
				$(service_mast_id).empty();

				$(service_mast_id).append('<option value="0">Select Service Name</option>');
				$.each(res,function(key,value){
					$('#service_mast').append('<option value="'+value.service_code+'" '+(old_service_code == value.service_code ? 'selected' : '')+' >'+value.service_desc+'</option>');
				});
				$(service_mast_id).append('<option value="other" '+(old_service_code == 'other' ? 'selected' : '')+'>Other</option>');
			}
			else{
				$(service_mast_id).empty();					
				$(service_mast_id).append('<option value="0">Select Service Name</option>');	
...
function service_client_mast_fetch(service_mast_id,service_catg_code,old_service_code=''){
	// console.log(old_service_code)
	if(service_catg_code !=''){
		$.ajax({
			type:'GET',
			url:'/service_client_mast_fetch/'+service_catg_code,
			success:function(res){
				// console.log(res);
				if(res.length !=0){
					$(service_mast_id).empty();

					$(service_mast_id).append('<option value="">Select Service Name</option>');
					$.each(res,function(key,value){
						if(value.service_mast !=null){
							$('#service_mast').append('<option value="'+value.id+'" '+( jQuery.inArray(value.id.toString(), old_service_code) != '-1' ? 'selected' : '')+' >'+value.service_mast.service_desc+ ' ('+value.service_rate+')'+'</option>');
						}
					...
					$(service_mast_id).empty();					
					$(service_mast_id).append('<option value="" disabled="disabled">Select Service Name</option>');
