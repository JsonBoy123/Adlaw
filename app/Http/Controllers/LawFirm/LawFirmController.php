...
class LawFirmController extends Controller
{
	public function index(){
		...
		// return $pen_todos;
        $cause_lists =  cause_list_fetch();
        $invoices =  InvoiceMast::where('user_id',Auth::user()->id)->count();

		return view('lawfirm.dashboard.index',compact('user','cases','running_cases','closed_cases','order_cases','direction_cases','transferred_cases','appointments','unbookings','booked','cancelled','todos','pen_todos','com_todos','mis_todos','clos_todos','awt_todos','hearings','moduleShow','cause_lists','invoices'));
