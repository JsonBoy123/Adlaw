...
class CaseMastController extends Controller
{
	...
	public function index(){
		...
		return view('case_management.case.index',compact('case_status','courts','case_types','clients'));
	}
		...
		if($data['court_type'] == '1'){
			...
			if($verify['no_catg'] == 'c_no'){
				$verify = $request->validate([
					...
					'c_d_number' => "required|string|min:5|max:24",
				],
				[
					'c_d_number.required' => 'The case number field is required.',	
					'c_d_number.max' => 'The case number may not be greater than 24 characters.',				
				]);
				...
				[
					...
					'c_d_number.max' => 'The diary number may not be greater than 24 characters.',
				]);
				$data['c_d_number'] 	= $verify['c_d_number'];
				$data['c_d_flag'] 		= 'd';
			}
	...
	public function case_details($case_id){
		...
		return view('case_management.case.case_details',compact('case'));
	}
	public function case_filter(Request $request){
		// return $request->all();
		$case_status 	= request()->case_status;
		...
		$q['case_mast.case_status'] = $case_status;

		if($cust_id !='0'){
			$q['cust_id'] = $cust_id ;
		}
		if($case_type_id !='0'){
			$q['case_type_id'] = $case_type_id;
		}
		if($court !='0'){
		...
		$cases = CaseLawyer::with(['member','case' => function($query) use($case_status,$q){
				$query->with(['client','court','casetype'])->where($q);
			}])->where('user_id1',$id)->where('deallocate_date',null)->get();
